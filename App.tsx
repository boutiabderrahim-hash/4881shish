...
const App: React.FC = () => {
    ...
    const handleLogin = (waiter: Waiter) => {
        ...
        const runningShift = shifts.find(s => !s.endTime && s.waiterId === waiter.id);
        if (runningShift) {
    ...
    const handleLogout = () => {
        if (activeShift) {
            alert('Debes cerrar tu turno antes de cerrar sesión.');
            return;
        }
    ...
    const updateOrderTotals = (order: Order): Order => {
        ...
        return { ...order, subtotal, tax, total, discount: order.discount ? {...order.discount, amount: discountAmount} : undefined };
    };
    ...
    return (
        <div className="bg-gray-800 text-white h-screen flex flex-col font-sans">
            <Header 
                ...
                onOpenDrawer={() => alert('Abrir caja no implementado')}
            ...
            {isPaymentModalOpen && currentOrder && (
                <PaymentModal
                    ...
                    onPartialPayment={() => alert('Pago parcial no implementado')}
                />
            )}
